...
void main() {
  ...
  print('📅 ATTENTION: Les décisions automatiques ne s\'affichent qu\'en fin d\'année (T3 ou S2)');
  print('=' * 60);
  
  // Test des différentes moyennes et décisions correspondantes
  final testCases = [
    {'moyenne': 18.5, 'expected': 'Admis en classe supérieure avec félicitations'},
    {'moyenne': 16.2, 'expected': 'Admis en classe supérieure avec félicitations'},
    {'moyenne': 15.8, 'expected': 'Admis en classe supérieure avec encouragements'},
    {'moyenne': 14.5, 'expected': 'Admis en classe supérieure avec encouragements'},
    {'moyenne': 13.2, 'expected': 'Admis en classe supérieure'},
    {'moyenne': 12.0, 'expected': 'Admis en classe supérieure'},
    {'moyenne': 11.5, 'expected': 'Admis en classe supérieure avec avertissement'},
    {'moyenne': 10.0, 'expected': 'Admis en classe supérieure avec avertissement'},
  ...
  final decisionFallback = _getDecisionAutomatiqueWithFallback(null, 13.5, 'Trimestre 3');
  print('✅ Moyenne annuelle: null, Moyenne générale: 13.5, T3 → $decisionFallback');
  
  // Test avec moyenne annuelle disponible
  final decisionAnnuelle = _getDecisionAutomatiqueWithFallback(15.8, 13.5, 'Semestre 2');
  print('✅ Moyenne annuelle: 15.8, Moyenne générale: 13.5, S2 → $decisionAnnuelle');
  
  print('\n🚫 Test des périodes non-automatiques :');
  print('-' * 60);
  
  // Test avec période non-automatique
  final decisionT1 = _getDecisionAutomatiqueWithFallback(15.8, 13.5, 'Trimestre 1');
  print('❌ Moyenne annuelle: 15.8, Moyenne générale: 13.5, T1 → $decisionT1');
  
  final decisionS1 = _getDecisionAutomatiqueWithFallback(15.8, 13.5, 'Semestre 1');
...
String _getDecisionAutomatiqueWithFallback(double? moyenneAnnuelle, double moyenneGenerale, String selectedTerm) {
