...
class _StudentProfilePageState extends State<StudentProfilePage> with TickerProviderStateMixin {
            ...
            children: [
              ...
              Container(
                ...
                child: TabBar(
                  ...
                  tabs: [
                    Tab(
                      icon: Icon(Icons.person, color: _tabController.index == 0 ? Colors.white : theme.textTheme.bodyMedium?.color),
                      text: 'Infos',
                    ),
                    Tab(
                      icon: Icon(Icons.payment, color: _tabController.index == 1 ? Colors.white : theme.textTheme.bodyMedium?.color),
                      text: 'Paiements',
                    ),
                    Tab(
                      icon: Icon(Icons.article, color: _tabController.index == 2 ? Colors.white : theme.textTheme.bodyMedium?.color),
                      text: 'Bulletins',
                    ),
                  ],
                ),
      ...
      child: SingleChildScrollView(
        ...
        children: [
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (widget.student.matricule != null && widget.student.matricule!.isNotEmpty) ...[
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    _buildStatusChip('${widget.student.className}', Icons.class_, theme),
                    const SizedBox(width: 12),
                    _buildStatusChip(widget.student.gender == 'M' ? 'Garçon' : 'Fille', 
                        widget.student.gender == 'M' ? Icons.male : Icons.female, theme),
                  ],
                ),
              ],
            ),
          ...
          _buildInfoSection('Informations Personnelles', [
            _buildInfoCard('Numéro de Matricule', widget.student.matricule ?? 'Non attribué', Icons.badge, theme),
            _buildInfoCard('Date de Naissance', _formatDate(widget.student.dateOfBirth), Icons.cake, theme),
            _buildInfoCard('Âge', _calculateAge(widget.student.dateOfBirth), Icons.calendar_today, theme),
            _buildInfoCard('Genre', widget.student.gender == 'M' ? 'Garçon' : 'Fille', 
                widget.student.gender == 'M' ? Icons.male : Icons.female, theme),
            _buildInfoCard('Statut', widget.student.status, Icons.person_pin, theme),
            _buildInfoCard('Adresse', widget.student.address, Icons.location_on, theme),
          ], theme),
          
          const SizedBox(height: 24),
          
          _buildInfoSection('Informations de Contact', [
            _buildInfoCard('Téléphone', widget.student.contactNumber, Icons.phone, theme),
            _buildInfoCard('Email', widget.student.email, Icons.email, theme),
            _buildInfoCard('Contact d\'urgence', widget.student.emergencyContact, Icons.emergency, theme),
          ], theme),
          
          const SizedBox(height: 24),
          
          _buildInfoSection('Informations Familiales', [
            _buildInfoCard('Nom du Tuteur', widget.student.guardianName, Icons.person, theme),
            _buildInfoCard('Contact du Tuteur', widget.student.guardianContact, Icons.phone, theme),
          ], theme),
          
          const SizedBox(height: 24),
          
          _buildInfoSection('Informations Académiques', [
            _buildInfoCard('Classe Actuelle', widget.student.className, Icons.school, theme),
            _buildInfoCard('Année d\'inscription', _getEnrollmentYear(), Icons.calendar_month, theme),
            if (widget.student.medicalInfo != null && widget.student.medicalInfo!.isNotEmpty)
              _buildInfoCard('Informations Médicales', widget.student.medicalInfo!, Icons.medical_services, theme),
          ], theme),
        ],
        ),
      ),
  ...
  Widget _buildPaymentsTab() {
    ...
    final totalPaid = _payments.where((p) => !p.isCancelled).fold(0.0, (sum, item) => sum + item.amount);
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Container(
                              ...
                              child: Icon(
                                payment.isCancelled ? Icons.cancel : Icons.receipt_long,
                                color: payment.isCancelled ? Colors.red : theme.colorScheme.primary,
                                size: 20,
                              ),
                            ),
                            const SizedBox(width: 12),
                        ...
                        if (payment.isCancelled && payment.cancelledAt != null) ...[
                          const SizedBox(height: 8),
                          Text(
                              ...
                              onPressed: () async {
                                ...
                                if (studentClass != null) {
                                  ...
                                  if (directoryPath != null) {
                                    ...
                                    showRootSnackBar(SnackBar(content: Text('Reçu enregistré dans $directoryPath'), backgroundColor: Colors.green));
                                  }
                                }
                              },
  ...
  Widget _buildReportCardsTab() {
    ...
    return Container(
      ...
      child: _reportCards.isEmpty ? Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(
      ...
      itemBuilder: (context, index) {
        ...
        final moyenneGenerale = reportCard['moyenne_generale']?.toDouble() ?? 0.0;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          if ((reportCard['school_ministry'] ?? '').toString().trim().isNotEmpty)
                            Text((reportCard['school_ministry'] as String).toUpperCase(), style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold)),
                          if ((reportCard['school_inspection'] ?? '').toString().trim().isNotEmpty)
                            Text('Inspection: ${reportCard['school_inspection']}', style: theme.textTheme.bodySmall),
                        ],
                      ),
                    ),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(((reportCard['school_republic'] ?? 'RÉPUBLIQUE') as String).toUpperCase(), style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold)),
                          if ((reportCard['school_republic_motto'] ?? '').toString().trim().isNotEmpty)
                            Text(reportCard['school_republic_motto'], style: theme.textTheme.bodySmall?.copyWith(fontStyle: FontStyle.italic)),
                          if ((reportCard['school_education_direction'] ?? '').toString().trim().isNotEmpty)
                            Text("Direction de l'enseignement: ${reportCard['school_education_direction']}", style: theme.textTheme.bodySmall),
                        ],
                      ),
                    ),
                  ],
                ...
                Row(
                  children: [
                    Expanded(child: Text('Nom: ${widget.student.name}', style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600))),
                    Expanded(child: Text('Date de naissance: ${_fmtArchivedDate(reportCard['student_dob'])}', style: theme.textTheme.bodySmall)),
                    Expanded(child: Text('Statut: ${reportCard['student_status'] ?? ''}', style: theme.textTheme.bodySmall)),
                  ],
                ),
                const SizedBox(height: 8),
                if (((reportCard['student_photo_path'] ?? '').toString().trim().isNotEmpty) &&
                    File((reportCard['student_photo_path'] as String)).existsSync())
                  Align(
                    ...
                    child: Container(
                      ...
                      child: Image.file(File(reportCard['student_photo_path'] as String), fit: BoxFit.cover),
                    ),
                  ),
                const SizedBox(height: 8),
                // Header
                Row(
                  children: [
                    ...
                    if (((reportCard['exaequo'] is int) && (reportCard['exaequo'] as int) == 1) || reportCard['exaequo'] == true)
                      Container(
                ...
                FutureBuilder<Map<String, dynamic>>(
                  future: () async {
                    num avg = (reportCard['moyenne_generale'] as num?) ?? 0;
                    ...
                    if (avg == 0) {
                      try {
                        ...
                        if (idx >= 0 && idx < moyennesParPeriode.length && moyennesParPeriode[idx] != null) {
                          avg = (moyennesParPeriode[idx] as double);
                        }
                      } catch (_) {}
                    }
                    // Fallback: compute rank/nb from archived grades for this class/year/term
                    if (rank == 0 || nb == 0 || avg == 0 || !isExAequo) {
                      try {
                        final archived = await _dbService.getArchivedGrades(
                        ...
                        for (final g in archived.where((g) => g.term == term)) {
                          final s = sums.putIfAbsent(g.studentId, () => {'n': 0.0, 'c': 0.0});
                          if (g.maxValue > 0 && g.coefficient > 0) {
                            s['n'] = (s['n'] ?? 0) + ((g.value / g.maxValue) * 20) * g.coefficient;
                            s['c'] = (s['c'] ?? 0) + g.coefficient;
                          }
                        }
                        final List<MapEntry<String, double>> avgs = sums.entries
                            .map((e) => MapEntry(e.key, (e.value['c'] ?? 0) > 0 ? (e.value['n']! / e.value['c']!) : 0.0))
                            .toList();
                        avgs.sort((a, b) => b.value.compareTo(a.value));
                        nb = avgs.length;
                        final sid = reportCard['studentId'] as String?;
                        final self = avgs.firstWhere((e) => e.key == sid, orElse: () => const MapEntry('', 0.0));
                        final double myAvg = self.value;
                        const double eps = 0.001;
                        // Rang ex æquo: 1 + nombre d'élèves avec une moyenne strictement supérieure
                        rank = 1 + avgs.where((e) => (e.value - myAvg) > eps).length;
                        // Ex æquo si d'autres élèves ont la même moyenne (tolérance eps)
                        isExAequo = avgs.where((e) => (e.value - myAvg).abs() < eps).length > 1;
                        if (avg == 0 && myAvg > 0) avg = myAvg;
                      } catch (_) {}
                    }
                    return {'avg': avg, 'rank': rank, 'nb': nb, 'exaequo': isExAequo};
                  }(),
                  builder: (context, snap) {
                    final num avg = snap.data?['avg'] ?? (reportCard['moyenne_generale'] as num? ?? 0);
                    final num rank = snap.data?['rank'] ?? (reportCard['rang'] as num? ?? 0);
                    final num nb = snap.data?['nb'] ?? (reportCard['nb_eleves'] as num? ?? 0);
                    final bool exaequo = (snap.data?['exaequo'] as bool?)
                        ?? ((reportCard['exaequo'] is int) ? (reportCard['exaequo'] as int) == 1 : (reportCard['exaequo'] == true));
                    return Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: Column(
                              children: [
                                Text('Moyenne Générale', style: theme.textTheme.bodyMedium?.copyWith(color: theme.textTheme.bodyMedium?.color?.withOpacity(0.7))),
                                const SizedBox(height: 4),
                                Text(avg.toStringAsFixed(2), style: theme.textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.bold, color: gradeColor)),
                              ],
                            ),
                          ),
                          Container(width: 1, height: 40, color: gradeColor.withOpacity(0.3)),
                          Expanded(
                            child: Column(
                              children: [
                                Text('Rang', style: theme.textTheme.bodyMedium?.copyWith(color: theme.textTheme.bodyMedium?.color?.withOpacity(0.7))),
                                const SizedBox(height: 4),
                                Text(
                                  exaequo ? '${rank.toInt()} (ex æquo) / ${nb.toInt()}' : '${rank.toInt()} / ${nb.toInt()}',
                ...
                Wrap(
                  ...
                  runSpacing: 12,
                  children: [
                    _buildStatCard('Moyenne Classe', '${reportCard['moyenne_generale_classe']?.toStringAsFixed(2) ?? '-'}', Icons.group, theme),
                    _buildStatCard('Plus Forte', '${reportCard['moyenne_la_plus_forte']?.toStringAsFixed(2) ?? '-'}', Icons.trending_up, theme),
                    _buildStatCard('Plus Faible', '${reportCard['moyenne_la_plus_faible']?.toStringAsFixed(2) ?? '-'}', Icons.trending_down, theme),
                    _buildStatCard('Moyenne Annuelle', '${reportCard['moyenne_annuelle']?.toStringAsFixed(2) ?? '-'}', Icons.calendar_today, theme),
                  ],
                ),
                      ...
                      onPressed: () async {
                        ...
                        try {
                          ...
                          final List<double> avgs = sums.entries
                              .map((e) => (e.value['c'] ?? 0) > 0 ? (e.value['n']! / e.value['c']!) : 0.0)
                              .toList();
                        ...
                        if (directoryPath != null) {
                          ...
                          final file = File('$directoryPath/$fileName');
                          await file.writeAsBytes(pdfBytes);
                          showRootSnackBar(SnackBar(content: Text('Bulletin enregistré dans $directoryPath'), backgroundColor: Colors.green));
                        }
                      },
                      icon: Icon(Icons.picture_as_pdf, size: 18),
                      label: Text('Télécharger PDF'),
